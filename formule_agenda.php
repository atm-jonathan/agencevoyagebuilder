...
/**
 *  \file       formule_agenda.php
 *  \ingroup    formulevoyage
 *  \brief      Tab of events on Formule
 */
...
$hookmanager->initHooks(array('formuleagenda', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->formulevoyage->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($object->id > 0) {
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		$cachekey = 'count_events_formule_'.$object->id;
		$nbEvent = dol_getcache($cachekey);

		print_barre_liste($langs->trans("ActionsOnFormule").(is_numeric($nbEvent) ? '<span class="opacitymedium colorblack paddingleft">('.$nbEvent.')</span>': ''), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 1);
		*/
		print_barre_liste($langs->trans("ActionsOnFormule"), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 0);
