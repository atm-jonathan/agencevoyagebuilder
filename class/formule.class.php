...
/**
 * \file        class/formule.class.php
 * \ingroup     formulevoyage
 * \brief       This file is a CRUD class file for Formule (Create/Read/Update/Delete)
 */
 ...
 * Class for Formule
 */
class Formule extends CommonObject
{
	/**
	 * @var string ID of module.
	 */
	public $module = 'formulevoyage';

	/**
	 * @var string ID to identify managed object.
	 */
	public $element = 'formule';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'formulevoyage_formule';
	...
	public $fields=array(
		...
		'description' => array('type'=>'text', 'label'=>'Description', 'enabled'=>'1', 'position'=>2, 'notnull'=>1, 'visible'=>1, 'validate'=>'1',),
		...
		'status' => array('type'=>'integer', 'label'=>'Status', 'enabled'=>'1', 'position'=>2000, 'notnull'=>1, 'visible'=>2, 'index'=>1, 'validate'=>'1',),
	...
	//  */
	// public $table_element_line = 'formulevoyage_formuleline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_formule';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Formuleline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Formule', 'fk_element'=>'fk_formule'));
	...
	//  */
	// protected $childtablesoncascade = array('formulevoyage_formuledet');

	// /**
	//  * @var FormuleLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formulevoyage','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->formulevoyage->formule->formule_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'formule/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'formule/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->formulevoyage->dir_output.'/formule/'.$oldref;
				$dirdest = $conf->formulevoyage->dir_output.'/formule/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->formulevoyage->dir_output.'/formule/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formulevoyage','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('formulevoyage','formulevoyage_advance','validate'))))
		 {
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowFormule")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Formule").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/formulevoyage/formule_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new FormuleLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_formule = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('FORMULEVOYAGE_MYOBJECT_ADDON')) {
			...
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/formulevoyage/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/formulevoyage/doc/";
