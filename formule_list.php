...
/**
 *   	\file       formule_list.php
 *		\ingroup    formulevoyage
 *		\brief      List page for formule
 */
...
require_once __DIR__.'/class/formule.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('formulevoyage', 'formule', 'write');
	$permissiontodelete = $user->hasRight('formulevoyage', 'formule', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("formulevoyage")) {
	accessforbidden('Module formulevoyage not enabled');
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Formule';
	$objectlabel = 'Formule';
	$uploaddir = $conf->formulevoyage->dir_output;
...
$title = $langs->trans("Formules");
//$help_url = "EN:Module_Formule|FR:Module_Formule_FR|ES:Módulo_Formule";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/formulevoyage/formule_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/formulevoyage/formule_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendFormuleRef";
$modelmail = "formule";
$objecttmp = new Formule($db);
...
// Fields title label
// --------------------------------------------------------------------
$cssKanban = (GETPOST('mode', 'aZ09') == 'kanban') ? 'titleNone': '';
print '<tr class="liste_titre '.$cssKanban.'">';
// Action column
if (getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
