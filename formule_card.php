...
/**
 *   	\file       formule_card.php
 *		\ingroup    formulevoyage
 *		\brief      Page to create/edit/view formule
 */
...
$object = new Formule($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction = $conf->formulevoyage->dir_output.'/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('formulecard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('formulevoyage', 'formule', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('formulevoyage', 'formule', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('formulevoyage', 'formule', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->formulevoyage->multidir_output[isset($object->entity) ? $object->entity : 1].'/formule';
...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Formule")), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->formulevoyage->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('formulevoyage:Formule', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('formule'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/formulevoyage/formule_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->formulevoyage->dir_output;
	$trackid = 'formule'.$object->id;
