...
/**
 * \file    test/phpunit/FormulevoyageFunctionalTest.php
...
/**
 * Class FormulevoyageFunctionalTest
 *
 ...
 *
 * @package Testformulevoyage
 */
class FormulevoyageFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 5000001; // TODO: autodetect?
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modFormulevoyage')->click();
		} else {
			// Disable the module
			$this->byHref('modFormulevoyage')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/formulevoyage/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('formulevoyage/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/formulevoyage/admin/about.php');
		$this->authenticate();
		return $this->assertContains('formulevoyage/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('formulevoyagewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modFormulevoyage_FormulevoyageTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modFormulevoyage_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
